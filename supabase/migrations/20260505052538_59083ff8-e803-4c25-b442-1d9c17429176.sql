CREATE OR REPLACE FUNCTION public.create_self_service_trial(
  p_full_name text, p_org_name text, p_distributors_count integer,
  p_phone text, p_whatsapp text,
  p_base_currency_code text DEFAULT NULL::text,
  p_base_currency_name text DEFAULT NULL::text,
  p_base_currency_symbol text DEFAULT NULL::text,
  p_secondary_currency_code text DEFAULT NULL::text,
  p_secondary_currency_name text DEFAULT NULL::text,
  p_secondary_currency_symbol text DEFAULT NULL::text,
...
BEGIN
  ...
  INSERT INTO public.organizations(name) VALUES (trim(p_org_name)) RETURNING id INTO v_org_id;
  ...
  ON CONFLICT (id) DO UPDATE SET
    full_name = EXCLUDED.full_name, role = 'OWNER',
  ...
  IF p_base_currency_code IS NOT NULL AND length(trim(p_base_currency_code)) > 0 THEN
    ...
    UPDATE public.org_currencies SET is_base = false
     WHERE organization_id = v_org_id AND is_base = true AND currency_code <> v_base_code;
    ...
    IF p_secondary_currency_code IS NOT NULL
      ...
      -- p_exchange_rate الآن يمثّل: 1 USD = X SYP مباشرة (يطابق تبويب «العملات والصرف»)
      IF p_exchange_rate IS NOT NULL AND p_exchange_rate > 0 THEN
        INSERT INTO public.exchange_rates(
          ...
          v_org_id, 'USD', 'SYP', p_exchange_rate,
