  ...
  return useOfflineQuery({
    ...
    queryFn: async (): Promise<Payment[]> => {
      ...
      const buildQuery = (from: number, to: number) => {
        let q = supabase
          .from('collections')
          .select('id,sale_id,customer_id,amount,notes,is_reversed,reverse_reason,created_at,organization_id,collected_by,direction,currency,original_amount,exchange_rate,sales(customer_name),customers(name)')
          .order('created_at', { ascending: false })
          .range(from, to);
        if (orgId && role !== UserRole.DEVELOPER) q = q.eq('organization_id', orgId);
        return q;
