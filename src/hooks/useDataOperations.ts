...
export const transformProduct = (p: any): Product => ({
  ...
  packConsumerPrice: Number(p.pack_consumer_price ?? 0),
  unitsPerPack: Number(p.units_per_pack ?? 1) || 1,
  pricingUnit: (p.pricing_unit === 'PACK' ? 'PACK' : 'PIECE'),
  stockDisplayUnit: (p.stock_display_unit === 'PACK' ? 'PACK' : p.stock_display_unit === 'BOTH' ? 'BOTH' : 'PIECE'),
  allowPackSales: Boolean(p.allow_pack_sales ?? false),
  allowPieceSales: Boolean(p.allow_piece_sales ?? true),
  stock: p.stock,
