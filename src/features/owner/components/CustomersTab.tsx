...
const CustomersTab: React.FC = () => {
  ...
  const getStatusLabel = (balance: number) => {
    if (balance > 0) return { text: 'عليه ذمة', color: 'bg-destructive/10 text-destructive' };
    return { text: 'لا توجد ذمم', color: 'bg-success/10 text-success' };
  };
    ...
    <div className="space-y-3 animate-fade-in">
      ...
      <div className="bg-gradient-to-br from-destructive to-destructive/80 p-6 rounded-3xl text-destructive-foreground shadow-lg">
        <div className="flex justify-between items-start">
          <div>
            <div className="flex items-center gap-1.5 mb-1">
              <p className="text-xs opacity-80">إجمالي الذمم</p>
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent side="bottom">
                    <p className="text-xs">مجموع ديون الزبائن (لا يشمل أرصدة سالبة)</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
          ...
          <div className="text-left">
            <p className="text-xs opacity-80 mb-1">زبائن بذمم</p>
        ...
        <div className="mt-4 pt-3 border-t border-destructive-foreground/20 space-y-1.5 text-xs">
          ...
          <div className="flex justify-between">
            <span className="opacity-80">عدد الزبائن الكلي</span>
      ...
      {payments.length > 0 && (
        <div className="space-y-2">
          <h3 className="font-bold text-foreground text-sm flex items-center gap-2 px-2 mt-4">
            <Wallet className="w-4 h-4 text-success" /> سجل التحصيلات
          </h3>
          {payments.filter(p => !p.isReversed).slice(0, 10).map(p => {
            ...
            return (
              <div key={p.id} className="bg-card p-3 rounded-2xl shadow-sm flex items-center justify-between">
                ...
                <p className="font-black text-success">{p.amount.toLocaleString()} {CURRENCY}</p>
              </div>
            );
          })}
        </div>
