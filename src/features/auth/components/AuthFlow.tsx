...
const AuthFlow: React.FC<AuthFlowProps> = ({ onAuthComplete }) => {
  ...
  const checkUserProfile = useCallback(async (userId: string, user: any) => {
    try {
      ...
      setAuthState({ type: 'error', message: isTimeout ? t('auth.verifyTimeout') : err.message || t('auth.verifyError'), canRetry: true });
    }
        ...
        return (
          <div className="flex flex-col items-center justify-center py-12 space-y-4">
            ...
            <div className="text-center space-y-2 animate-in fade-in duration-300">
                <p className="text-xs text-muted-foreground/70">{t('auth.takingLong')}</p>
      ...
      case 'access_denied':
        return (
          <div className="space-y-6 text-center py-8">
            ...
            <button onClick={handleLogout} className="px-8 py-3 bg-muted text-foreground rounded-2xl font-bold text-sm hover:bg-muted/80 transition-colors">
              {t('common.logout')}
            </button>
          </div>);
        ...
        return (
          <div className="space-y-6 text-center py-8">
            ...
            <div className="flex flex-col gap-3 items-center">
              {authState.canRetry &&
              <button onClick={handleRetry} className="px-8 py-3 bg-primary text-primary-foreground rounded-2xl font-bold text-sm hover:bg-primary/90 transition-colors flex items-center gap-2">
        ...
        return (
          <div className="space-y-6">
            ...
            <GoogleSignInButton onError={handleAuthError} oauthInProgress={oauthPending}
              loadingText={oauthPending ? t('auth.returningFromGoogle') : undefined} />
            <div className="flex items-center gap-3">
              <div className="flex-1 h-px bg-border" />
              <span className="text-xs text-muted-foreground font-bold">{t('common.or')}</span>
              <div className="flex-1 h-px bg-border" />
            </div>
  ...
  return (
    <div className="min-h-screen flex flex-col font-tajawal relative bg-background" dir={document.documentElement.dir || 'rtl'}>
