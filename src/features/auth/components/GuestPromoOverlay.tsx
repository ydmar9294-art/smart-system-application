...
const GuestPromoOverlay: React.FC = () => {
  ...
  useEffect(() => {
    // Show first time after interval
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[9998] flex items-center justify-center p-6" dir="rtl">
      {/* Backdrop */}
      <div
      ...
      <div className="relative z-10 w-full max-w-sm animate-in zoom-in-95 fade-in duration-300">
        <div
          className="rounded-[2.5rem] overflow-hidden shadow-2xl p-6 space-y-5"
          style={{
        ...
        >
          ...
          <button
            onClick={() => setVisible(false)}
            className="absolute top-4 left-4 p-2 rounded-xl hover:bg-muted transition-colors"
          >
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              للحصول على الوصول الكامل، تواصل مع الشركة لحجز ترخيص تجريبي وجلسة شخصية لتحديد احتياجاتك.
            </p>
          </div>
          ...
          <button
            onClick={() => setVisible(false)}
            className="w-full text-center text-xs text-muted-foreground/60 hover:text-muted-foreground transition-colors font-bold"
          >
            إغلاق
          </button>
        </div>
