...
const LicenseActivation: React.FC<LicenseActivationProps> = ({ userId, email, fullName, onSuccess, onLogout }) => {
  ...
  const handleContactSupport = () => {
    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(t('activation.paymentNote') + ' ' + email)}`, '_blank');
  };
  ...
  const handleActivation = async () => {
    ...
    try {
      ...
      if (trimmedCode.startsWith('EMP-')) {
        ...
        if (!result.success) { setError(result.message || t('activation.activationFailedAccount')); return; }
      } else {
        ...
        if (rpcError) throw rpcError;
        const result = data as { success: boolean; error?: string; message?: string };
        if (!result.success) { setError(result.message || t('activation.activationFailedLicense')); return; }
      }
      onSuccess();
    } catch (err: any) {
      console.error('[LicenseActivation] Error:', err);
      setError(err.message || t('activation.activationFailed'));
  ...
  const getCodeTypeInfo = () => {
    switch (codeType) {
      ...
      case 'employee': return { icon: User, label: t('activation.employeeCode'), description: t('activation.employeeCodeDesc'), color: 'text-success', bgColor: 'bg-success/10', borderColor: 'border-success/30', glowColor: 'shadow-success/20' };
      default: return { icon: Key, label: t('activation.codeLabel'), description: t('activation.enterCodeToContinue'), color: 'text-muted-foreground', bgColor: 'bg-muted', borderColor: 'border-border', glowColor: '' };
    }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        <label className="text-sm font-bold text-foreground flex items-center gap-2"><Key className="w-4 h-4 text-primary" />{t('activation.codeLabel')}</label>
        <div className="relative">
          <input type="text" placeholder={t('activation.codePlaceholder')} value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} disabled={loading}
        ...
        {showPayment && (
          <div className="p-4 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30 rounded-2xl border border-green-200 dark:border-green-800 space-y-4 animate-in slide-in-from-top duration-300">
            ...
            <div className="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg">
              ...
              <span className="text-xs text-blue-700 dark:text-blue-300 font-bold">{t('activation.awaitingPayment')}</span>
            </div>
            ...
            <button onClick={handleContactSupport}
              ...
              <MessageCircle className="w-4 h-4" />{t('activation.contactFinance')}
            </button>
          </div>
        )}
      </div>
