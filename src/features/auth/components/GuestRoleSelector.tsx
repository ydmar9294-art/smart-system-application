...
const ROLE_ICONS: Record<string, React.ReactNode> = {
  ...
  'salesManager': <BarChart3 className="w-6 h-6" />,
  'accountant': <Calculator className="w-6 h-6" />,
  'warehouseKeeper': <Warehouse className="w-6 h-6" />,
  'fieldAgent': <Truck className="w-6 h-6" />,
};

const ROLE_COLORS: Record<string, string> = {
  'owner': 'from-primary to-primary/70',
  'salesManager': 'from-success to-success/70',
  'accountant': 'from-warning to-warning/70',
  'warehouseKeeper': 'from-[hsl(280,60%,55%)] to-[hsl(280,60%,40%)]',
  'fieldAgent': 'from-destructive to-destructive/70',
};
        ...
        <div className="rounded-[2rem] overflow-hidden shadow-2xl"
          ...
          <div className="px-6 pt-6 pb-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-black text-foreground">{t('guest.previewMode')}</h3>
                <p className="text-xs text-muted-foreground">{t('guest.selectRole')}</p>
              </div>
            </div>
          ...
          <div className="px-5 pb-6 space-y-2.5">
            {GUEST_ROLES.map((gr) => (
              <button key={gr.key} onClick={() => onSelect(gr)}
                ...
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${ROLE_COLORS[gr.key]} flex items-center justify-center text-white shadow-lg`}>
                  {ROLE_ICONS[gr.key]}
                </div>
                <div className="flex-1 text-start">
                  <p className="font-bold text-foreground text-sm">{t(`roles.${gr.key}`)}</p>
                  <p className="text-[11px] text-muted-foreground">{t('guest.dashboardPreview')}</p>
                </div>
          ...
          <div className="px-6 pb-5">
            <p className="text-[10px] text-muted-foreground/60 text-center leading-relaxed">{t('guest.readOnlyNote')}</p>
          </div>
        </div>
      </div>
    </div>,
