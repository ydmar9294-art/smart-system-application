...
const CollectionTab: React.FC<CollectionTabProps> = ({ selectedCustomer, onQueueAction, isOnline, localSales }) => {
  ...
  const handleCollect = async () => {
    ...
    try {
      ...
      addNotification(
        isOnline ? 'تم التحصيل بنجاح' : 'تم حفظ التحصيل — ستتم المزامنة عند عودة الإنترنت',
        'success'
      );
    } catch (err: any) {
  ...
  return (
    <div className="p-5 space-y-5">
      ...
      {/* Submit Button */}
      <button onClick={handleCollect} disabled={loading || !amount || !selectedSaleId}
        ...
        {loading ? (
          <><Loader2 className="w-6 h-6 animate-spin" /> جارٍ التحصيل...</>
        ) : (
          <><DollarSign className="w-6 h-6" /> توثيق سند القبض {!isOnline && <WifiOff className="w-4 h-4 opacity-60" />}</>
        )}
      </button>
