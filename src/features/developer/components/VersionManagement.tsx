 ...
 * Allows setting required version, update URL, force update toggle, and release notes.
 */
...
const VersionManagement: React.FC = () => {
  ...
  const [newMinRequired, setNewMinRequired] = useState('1.0.0');
  ...
  const handleAddVersion = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('app_versions').insert({
      ...
      min_required_version: newMinRequired.trim(),
  ...
  return (
    <div className="space-y-4">
      {/* Toast */}
      {toast && (
        <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-[999] px-5 py-3 rounded-xl font-bold text-sm shadow-lg flex items-center gap-2 animate-fade-in ${
      ...
      <div className="flex justify-between items-center">
        <h3 className="font-black text-foreground flex items-center gap-2">
          <Smartphone size={18} className="text-primary" /> إدارة إصدارات التطبيق
        </h3>
        <button
      ...
      {/* Add Form */}
      {showAddForm && (
        <div className="bg-muted/50 rounded-2xl p-5 border space-y-4">
          ...
          <form onSubmit={handleAddVersion} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-bold text-muted-foreground block mb-1">المنصة</label>
              ...
              <div>
                <label className="text-xs font-bold text-muted-foreground block mb-1">رقم الإصدار</label>
                <input value={newVersionName} onChange={e => setNewVersionName(e.target.value)}
                  placeholder="1.2.0" className="input-field !py-2.5 text-sm" required dir="ltr" />
              </div>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-bold text-muted-foreground block mb-1">كود الإصدار (Version Code)</label>
                <input type="number" value={newVersionCode} onChange={e => setNewVersionCode(Number(e.target.value))}
                  className="input-field !py-2.5 text-sm" min={1} dir="ltr" />
              </div>
              <div>
                <label className="text-xs font-bold text-muted-foreground block mb-1">أدنى إصدار مطلوب</label>
                <input value={newMinRequired} onChange={e => setNewMinRequired(e.target.value)}
                  placeholder="1.0.0" className="input-field !py-2.5 text-sm" dir="ltr" />
              </div>
            </div>

            <div>
              <label className="text-xs font-bold text-muted-foreground block mb-1">رابط التحديث</label>
              <input value={newUpdateUrl} onChange={e => setNewUpdateUrl(e.target.value)}
                placeholder="https://play.google.com/store/apps/..." className="input-field !py-2.5 text-sm" dir="ltr" />
            </div>

            <div>
              <label className="text-xs font-bold text-muted-foreground block mb-1">ملاحظات الإصدار</label>
            ...
            <div className="flex gap-2">
              <button type="submit" disabled={saving === 'new'}
                ...
                {saving === 'new' ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />}
                حفظ
              </button>
              <button type="button" onClick={() => setShowAddForm(false)}
                className="px-6 py-3 bg-muted text-muted-foreground rounded-xl font-bold text-sm">
                إلغاء
              </button>
            </div>
          </form>
      ...
      {versions.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground">
          ...
          <p className="text-sm mt-1">أضف إصدار جديد للبدء بإدارة التحديثات</p>
        </div>
      ) : (
...
const VersionCard: React.FC<{
  ...
  onDelete: () => void;
}> = ({ version, saving, onToggleForce, onUpdateField, onDelete }) => {
  ...
  return (
    <div className={`p-4 rounded-2xl border transition-all ${version.force_update ? 'border-destructive/30 bg-destructive/5' : 'border-border bg-card'}`}>
      ...
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-bold text-muted-foreground">التحديث الإجباري</span>
        <button onClick={onToggleForce} className="flex items-center gap-2">
          {version.force_update 
            ? <ToggleRight size={28} className="text-destructive" />
            : <ToggleLeft size={28} className="text-muted-foreground" />}
          <span className={`text-xs font-bold ${version.force_update ? 'text-destructive' : 'text-muted-foreground'}`}>
            {version.force_update ? 'مفعّل' : 'معطّل'}
          </span>
        </button>
      </div>
      ...
      {editing && (
        <div className="mt-3 pt-3 border-t space-y-2">
          <div>
            <label className="text-[10px] font-bold text-muted-foreground">أدنى إصدار مطلوب</label>
            <input value={editMinVersion} onChange={e => setEditMinVersion(e.target.value)}
              className="input-field !py-2 text-xs mt-1" dir="ltr" />
          </div>
          <div>
