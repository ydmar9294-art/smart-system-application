...
interface WelcomeSplashProps { onComplete?: () => void; duration?: number; }
...
const WelcomeSplash: React.FC<WelcomeSplashProps> = ({ onComplete, duration = 3000 }) => {
  ...
  const messages = [
    { text: t('splash.msg1'), icon: <Shield className="w-5 h-5 text-primary" /> },
    { text: t('splash.msg2'), icon: <Lock className="w-5 h-5 text-primary" /> },
    { text: t('splash.msg3'), icon: <CheckCircle className="w-5 h-5 text-primary" /> },
  ];
  ...
  const playWelcomeSound = useCallback(() => {
    try {
      ...
      playTone(523.25, 0, 0.3, 0.12); playTone(659.25, 0.1, 0.3, 0.1); playTone(783.99, 0.2, 0.4, 0.08); playTone(1046.50, 0.35, 0.5, 0.06);
    } catch {}
  }, []);
  ...
  return (
    <div className={`fixed inset-0 z-[9999] bg-background flex flex-col items-center justify-center p-6 text-center transition-all duration-500 safe-area-x safe-area-bottom ${isExiting ? 'opacity-0 scale-105' : 'opacity-100 scale-100'}`}
      ...
      <div className="flex flex-col items-center gap-6 max-w-xs w-full animate-fade-in">
        ...
        <div className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-full">
          <Shield className="w-4 h-4 text-primary" /><span className="text-xs font-bold text-primary">{t('auth.secureConnection')}</span>
        </div>
        ...
        <div className="space-y-3 min-h-[80px] flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-muted-foreground font-bold">{t('splash.preparing')}</p>
        </div>
