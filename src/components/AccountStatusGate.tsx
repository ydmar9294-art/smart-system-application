...
interface AccountStatus { isActive: boolean; isSuspended: boolean; suspensionReason: string | null; }
...
const AccountStatusGate: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const checkAccountStatus = useCallback(async () => {
    ...
    try {
      ...
      const newStatus: AccountStatus = {
        ...
        suspensionReason: profile.is_active === false ? t('accountStatus.deactivated') : licenseMessage,
      };
    ...
    } catch { logger.warn('[AccountStatus] Check failed, keeping last known state', 'AccountStatus'); }
  ...
  if (status.isSuspended && user) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-6" dir={document.documentElement.dir || 'rtl'}>
        <div className="max-w-md w-full bg-card rounded-3xl shadow-2xl p-8 text-center space-y-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground leading-relaxed">{status.suspensionReason || t('accountStatus.suspendedDesc')}</p>
          </div>
          <div className="space-y-3">
            <button onClick={checkAccountStatus} disabled={checking || !navigator.onLine}
              ...
              {checking ? <><RefreshCw className="w-5 h-5 animate-spin" /> {t('accountStatus.checking')}</> :
               !navigator.onLine ? <><WifiOff className="w-5 h-5" /> {t('accountStatus.offline')}</> :
               <><RefreshCw className="w-5 h-5" /> {t('accountStatus.recheck')}</>}
            </button>
            <button onClick={logout} className="w-full bg-muted text-muted-foreground py-3 rounded-xl font-bold hover:bg-accent transition-colors">
              {t('common.logout')}
            </button>
          </div>
